...
const milestones = [
                    ...
                    <li key={i} className="flex items-start">
                        <div>
                            {item.checked ? (
                                ...
                                <FaCircle className="text-[0.5rem] mr-3 mt-2"/>
                            )}
                        </div>
                        <div>
                            {FixText(item.text)}
        ...
        description: (
            <ul className="space-y-3">
                {[
                    ...
                    {
                        text: 'XRPH & RLUSD token ready to use as a digital payment method for products and services within the ecosystem',
                        checked: true
                    },
                    ...
                    {text: '870K XRPH distributed as staking rewards in April', checked: true},
                    {text: '804K XRPH distributed as staking rewards in May', checked: true},
                ].map((item, i) => (
                    <li key={i} className="flex items-start">
                        <div>
                            {item.checked ? (
        ...
        description: (
            <ul className="space-y-3">
                {[
                    ...
                    {text: "Integrated XRP Healthcare Prescription Savings Card into XRPH AI App.", checked: true},
                    {text: "Pharmacy search and connect feature added to XRPH AI App.", checked: true},
                    {
                    ...
                    {text: "XRP Healthcare to be listed on a Canadian stock exchange (IPO)", checked: false},
                ].map((item, i) => (
                    <li key={i} className="flex items-start">
                        <div>
                            {item.checked ? (
                                <FaCheckDouble className="text-sm text-purple-500 mr-2 mt-1.5"/>
                            ) : (
                                <FaCircle className="text-[0.5rem] mr-3 mt-2"/>
                            )}
                        </div>
        ...
        description: (
            <ul className="space-y-3">
                {[
                    ...
                    {text: "Strengthened international partnerships to solidify the XRP Healthcare ecosystem", checked: false},
    ...
    },
    {
        ...
        description: (
            <ul className="space-y-3">
                {[
                    {text: "Increase the value of our portfolio by implementation of growth strategies", checked: false},
                    {text: "Expand into new markets and territories by diversifying the XRP Healthcare portfolio",checked: false},
                    {text: "Grow revenue and profitability of the company", checked: false},
                    {text: "Integrate targets post merger and expand output through CAPEX and streamlined operations", checked: false},
                    {text: "Second Strategic Acquisition Secured", checked: false},
                    {text: "XRPH token accepted for payment and services rendered at all XRP Healthcare pharmacies, medical centres and hospitals", checked: false},
                ].map((item, i) => (
                    <li key={i} className="flex items-start">
                        <div>
                            {item.checked ? (
        ...
        <>
            ...
            <div className="">
                ...
                <p>The XRPH token is a scarce asset, with only 100 million tokens ever created. The XRPH account has
                    ...
                    <br/><br/>Our focus remains on building trust and delivering long{FixText('-')}term value to our ecosystem,
                    ensuring that
